...
import { FunnelBuilder } from "@/widgets/funnel-builder";
import { routing } from "@/i18n/routing";
import { getCurrentUser, getServerClient } from "@/lib/supabase/server";
import { fetchProject } from "@/entities/project";

/**
 * Route-level metadata (ADR 0031): the localized "Funnels" title, which the locale
 * layout's title template wraps (e.g. "Funnels · CAPCOM — Product Analytics") for
...
/**
 * Funnels route (PR-5) — the second flagship aggregation surface. The project is
 * resolved as the signed-in user under RLS, so a project in another tenant simply
 * isn't returned and resolves to a 404 (no existence leaked), mirroring the trends
 * page. The interactive builder (client) reads its state from the URL (nuqs) and calls
 * the in-database `fn_funnel` aggregation under the caller's RLS (ADR 0084/0087).
 */
