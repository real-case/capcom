...
(ADR 0033/0058), via **visx** primitives (ADR 0084, below).
...
| PR-1  | Foundational domain ADRs           | 0081–0084       | decisions-first, human gate              |
...
| PR-5  | Funnels                            | 0085 (if any)   | ordered-step conversion in SQL           |
| PR-6  | Retention cohort grid              | —               | the signature dense infographic          |
| PR-7  | Segmentation                       | 0086            | flexible attribute/behavior rules       |
| PR-8  | Dashboards + saved reports         | —               | Server Actions, optimistic mutations     |
| PR-9  | AI natural-language query          | 0087 (if any)   | provider-agnostic advisory AI            |
...
The purest decisions-first PR: **documentation only, no product code.** Four records take the
corpus past the template's final ADR (0080):

- **ADR 0081 — Event & identity model + multitenancy.** `org → project`; the `event` shape
...
- **ADR 0082 — Aggregation strategy.** Trends, funnels, and retention are computed by
...
- **ADR 0083 — Event ingestion contract.** A single demonstration `ingest` route: Zod-validated
...
- **ADR 0084 — Charting primitive layer: visx + design tokens.** Low-level visx primitives
...
- **Ingest route** (ADR 0083): `POST` an event batch, Zod-validated, authorized by a
...
- **Migration `fn_event_trends`:** a SQL function returning time-bucketed event counts with an
  optional breakdown by a property (ADR 0082).
...
warrant a recorded decision, **ADR 0085** precedes it. Feature `funnel-builder` +
...
**ADR 0086** records the segment-definition model (attribute predicates + behavioral rules).
...
**sleeps without `AI_API_KEY`** with a graceful fallback. **ADR 0087** records the
...
| Design-token governance     | PR-4+ chart colors from tokens only (visx, ADR 0084)      |
